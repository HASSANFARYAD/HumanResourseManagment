...
//#region Dashboard
export const dashboardGraphDetail = createAsyncThunk(
  "dashboard/dashboardGraphData",
  async (_, { rejectWithValue, getState, dispatch }) => {
    ...
    try {
      const response = await axios.get(
        `${baseUrl}Analytics/GetDashboardGraphDetail`,
        config
      );
...
const adminSlices = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(dashboardGraphDetail.pending, (state, action) => {
      state.loading = true;
      state.appStatusCode = undefined;
      state.profileAppErr = undefined;
      state.profileAppErr = undefined;
    ...
    builder.addCase(dashboardGraphDetail.fulfilled, (state, action) => {
      state.loading = false;
      state.appStatus = false;
      state.appStatusCode = undefined;
      state.profileAppErr = undefined;
    ...
    builder.addCase(dashboardGraphDetail.rejected, (state, action) => {
      state.appErr = action?.payload?.message;
      state.loading = false;
      state.serverErr = undefined;
      state.appStatus = action?.payload?.status;
